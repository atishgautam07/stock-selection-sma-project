...
from stockSelProj.utils.common import get_size
import pandas as pd
from pathlib import Path
from stockSelProj.entity.config_entity import (DataIngestionConfig)
...
class DataIngestion:
    ...
    def getIndiaStocks(self):
        ...
        filtered_df = screener_df[
            ...
            ((screener_df['Earnings Growth'] > 0.05)|(screener_df['Earnings Growth'].isna())) &
    ...
    def persist(self):
        ...
        if os.path.exists(file_name):
            os.remove(file_name)
        self.ticker_df.to_parquet(os.path.join(self.config.root_dir,file_name), compression='brotli')
    
        file_name = 'indexes_df.parquet'
        if os.path.exists(file_name):
            os.remove(file_name)
        self.indexes_df.to_parquet(os.path.join(self.config.root_dir,file_name), compression='brotli')
    
        file_name = 'macro_df.parquet'
        if os.path.exists(file_name):
            os.remove(file_name)
        self.macro_df.to_parquet(os.path.join(self.config.root_dir,file_name), compression='brotli')

    def load(self):
        """Load files from the local directory"""
        self.ticker_df = pd.read_parquet(os.path.join(self.config.root_dir,'tickers_df.parquet'))
        self.macro_df = pd.read_parquet(os.path.join(self.config.root_dir,'macro_df.parquet'))
        self.indexes_df = pd.read_parquet(os.path.join(self.config.root_dir,'indexes_df.parquet'))