...
import pandas as pd
from stockSelProj.entity.config_entity import DataTransformationConfig
...
class DataTransformation:
        def __init__(self, config: DataTransformationConfig):
                ...
                self.tickers_df = pd.read_parquet(os.path.join(self.config.data_path, "tickers_df.parquet"))
                self.macro_df = pd.read_parquet(os.path.join(self.config.data_path, "macro_df.parquet"))
                self.indexes_df = pd.read_parquet(os.path.join(self.config.data_path, "indexes_df.parquet"))
        ...
        def persist(self):
                ...
                if os.path.exists(file_name):
                        os.remove(file_name)
                self.transformed_df.to_parquet(os.path.join(self.config.root_dir, file_name), compression='brotli')

        def load(self):
                """Load files from the local directory"""
                os.makedirs(self.config.root_dir, exist_ok=True)      
                self.transformed_df  = pd.read_parquet(os.path.join(self.config.root_dir,'transformed_df.parquet'))
